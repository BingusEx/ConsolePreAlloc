...
namespace Hooks::stl {
	...
	void write_detour(REL::RelocationID a_relId) {
		...
		if (!target.address()) {
			SKSE::stl::report_and_fail(fmt::format("Invalid target address for detour [{} + 0x{:X}]", a_relId.id(), a_relId.offset()));
		}
		...
		if (attachResult != NO_ERROR) {
			...
			SKSE::stl::report_and_fail(fmt::format("Detour Attach Failed [{} + 0x{:X}] - Error: {}", a_relId.id(), a_relId.offset(), attachResult));
		}

		LONG error = DetourTransactionCommit();
		if (error != NO_ERROR) {
			SKSE::stl::report_and_fail(fmt::format("Detour Write Failed [{} + 0x{:X}] - Error: {}", a_relId.id(), a_relId.offset(), error));
		}
	}
	...
	void write_detour_replace(REL::RelocationID a_relId) {
		...
		// Validate target address
		if (!target.address()) {
			SKSE::stl::report_and_fail(fmt::format("Invalid target address for detour replace [{} + 0x{:X}]", a_relId.id(), a_relId.offset()));
		}
		...
		if (attachResult != NO_ERROR) {
			DetourTransactionAbort();
			SKSE::stl::report_and_fail(fmt::format("Detour Replace Attach Failed [{} + 0x{:X}] - Error: {}", a_relId.id(), a_relId.offset(), attachResult));
		}

		LONG error = DetourTransactionCommit();
		if (error != NO_ERROR) {
			SKSE::stl::report_and_fail(fmt::format("Detour Replace Write Failed [{} + 0x{:X}] - Error: {}", a_relId.id(), a_relId.offset(), error));
		}
	}
	...
	void write_detour_vfunc(void* target) {
		...
		if (!vtable || !vtable[idx]) {
			SKSE::stl::report_and_fail(fmt::format("Detour vFunc Write Failed - Invalid vtable or function at index {}", idx));
		}
		...
		if (attachResult != NO_ERROR) {
			DetourTransactionAbort();
			SKSE::stl::report_and_fail(fmt::format("Detour vFunc Attach Failed at index {} - Error: {}", idx, attachResult));
		}
